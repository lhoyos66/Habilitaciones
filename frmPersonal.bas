VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} frmPersonal 
   Caption         =   "Listado de Personal"
   ClientHeight    =   3540
   ClientLeft      =   120
   ClientTop       =   470
   ClientWidth     =   9080.001
   OleObjectBlob   =   "frmPersonal.frx":0000
...
Public Sub LlenarListBoxConDatos(nombreHoja As String)
    ...
    ' Asegúrate de que la hoja tiene una tabla llamada "tblProveedores"
    Dim tbl As ListObject
    Set tbl = hoja.ListObjects("Personal")
    ...
    For i = 1 To tbl.ListRows.Count
        Me.ListBox1.AddItem tbl.DataBodyRange(i, 1).Value & " - " & tbl.DataBodyRange(i, 2).Value & " - " & tbl.DataBodyRange(i, 16).Value
    Next i
End Sub
...
Public Sub FiltrarLista(criterio As String)
    ...
    nombreHoja = "MASTER PERSONAL" ' Nombre de la hoja que contiene la tabla de proveedores
